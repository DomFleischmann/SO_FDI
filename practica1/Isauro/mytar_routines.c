...
int extractTar(char tarName[])
{
	...
	FILE* tarFile;
	stHeaderEntry *header;
	unsigned int nFiles, loop;
	
	// Checks that tarName exists and can be readed.
	...
	// We read the tar file header.
	header = readHeader(tarFile, &nFiles);
	
	// If no files to decompress - Error.
	if(nFiles <= 0) {
		fprintf(stderr, "Error, 0 files: ", tarName);
		perror(NULL);
		return (EXIT_FAILURE);
	}
	
	// for each file to decompress...
	for(loop =0 ; loop < nFiles; loop++) {
		FILE* destination;
		
		// We open the file..
		destination = fopen(header[loop].name, "w");
		if(destination == NULL) {
			fprintf(stderr, "The mtar file %s could not be opened: ", tarName);
			perror(NULL);
			return (EXIT_FAILURE);
		...
		// And we copy de data
		if(copynFile(tarFile, destination, header[loop].size) == -1) {
			fprintf(stderr, "Error ocurred while copying the file: ", tarName);
			free(header);
			fclose(destination);
			return EXIT_FAILURE;
		}
