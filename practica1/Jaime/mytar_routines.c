...
int extractTar(char tarName[])
{
  ...
  //read num files and store header in memory
  header =  readHeader(tarFile, &nFiles);
  
  //seek in mtar data area
  headersize = sizeof(header);

  //create new files with data and file name
  for( i = 0; i < nFiles; i++){

    if((cFile = fopen(header[i].name, "w+")) == NULL){

      fprintf(stderr,"Not possible to open tar file %s: \n",header[i].name);
      perror(NULL);
      fclose(tarFile);
      remove(tarName);
      for(j = 0; j < nFiles; j++){
    ...
    if(copynFile(tarFile,cFile,header[i].size) == -1){


      fprintf(stderr, "Not possible to open file \n");
      perror(NULL);
      //clean heap memory
      fclose(cFile);
      fclose(tarFile);
  ...
  fprintf(stdout, "mtar extracted successfully \n");
